const items = [
  ...
  function capitalizeNames() {
    ...
    const capitalizedArr = items.map((item) => {return  item.name.toUpperCase()});
  ...
  function calculateAllInventoryPrice() {
    ...
    const totalPrice = items.reduce((total,item) =>{
        return total + item.inventory * item.price;
    },0);
  ...
  function filterItemsByCategoryId(categoryId) {
    ...
    itemsArr = items.filter(item => item.categoryId === categoryId);
    return itemsArr;
  }
  ...
  function calculateTotalCartPrice() {
    ...
    for(let i=0; i < cart.length;i++)
      total += items.find(item => item.id == cart[i]).price;
